...
const ChessItem = ({player, type, beSelected, canBeMoveTo, onSelect, onMoveTo}) => {
    const onClick = () => {
        if (canBeMoveTo)
            onMoveTo();
        else if (type != ChessTypes.NONE)
            onSelect();
    };
    return (
        <div className={`chess-block ${beSelected ? 'selected' : ''} ${canBeMoveTo ? 'can-be-move-to' : ''}`}
             onClick={onClick}>
            {
...
const mapStateToProps = (state, {chessId}) => {
    return {
